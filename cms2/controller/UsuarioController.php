...
class UsuarioController
{
    ...
    public function entrar(){
        ...
        //Si ha pulsado el botón de acceder, tramito el formulario
            ...
            $rowset = $this->db->query("SELECT * FROM users WHERE usuario='$campo_usuario' AND activo=1 LIMIT 1");
            ...
            if ($usuario){
                ...
                if (password_verify($campo_clave,$usuario->clave)) {
                    ...
                    $_SESSION["secciones"] = $usuario->secciones;

                    //Guardo la fecha de último acceso
                    $ahora = new \DateTime("now", new \DateTimeZone("Europe/Madrid"));
                    $fecha = $ahora->format("Y-m-d H:i:s");
                    $this->db->exec("UPDATE users SET fecha_acceso='$fecha' WHERE usuario='$campo_usuario'");
    ...
    public function index(){
        ...
        $rowset = $this->db->query("SELECT * FROM users ORDER BY usuario ASC");
    ...
    public function activar($id){
        ...
        if ($usuario->activo == 1){
            ...
            $consulta = $this->db->exec("UPDATE users SET activo=0 WHERE id='$id'");
            ...
            //Activo el usuario
            $consulta = $this->db->exec("UPDATE users SET activo=1 WHERE id='$id'");
    ...
    public function editar($id){
        ...
        if (isset($_POST["guardar"])){
            ...
            $secciones = (filter_input(INPUT_POST, 'secciones', FILTER_SANITIZE_STRING) == 'on') ? 1 : 0;
            ...
            if ($id == "nuevo"){
                ...
                $this->db->exec("INSERT INTO users (usuario, clave, secciones, usuarios) VALUES ('$usuario','$clave_encriptada',$secciones,$usuarios)");
                ...
                ($cambiar_clave) ?
                    $this->db->exec("UPDATE users SET usuario='$usuario',clave='$clave_encriptada',secciones=$secciones,usuarios=$usuarios WHERE id='$id'") :
                    $this->db->exec("UPDATE users SET usuario='$usuario',secciones=$secciones,usuarios=$usuarios WHERE id='$id'");
